...
function listCues(results){
  ...
  var myOriginalDataset = results;
  var splitValuesArray = getFieldValues(myOriginalDataset);
    
  // convert values to binary
  var myDatasetBinary = convertToBinary(myOriginalDataset, splitValuesArray);
  console.log('Converted Dataset: '+JSON.stringify(myDatasetBinary, null, "  "));
  
    myDataset = myDatasetBinary;
                        ...
                        <ul class="stat_cue_header"> \
                          <li class="stats stat_cue unsortable">\
                            <table class="eval_table" id="stat_cue'+i+'"> \
                                ...
                                <tr class="row_to_delete"><th class="yes_label" colspan="2">max</th><td class="swap" colspan="3"><svg class="button_swap" height="20" width="40"><polyline class="left" points="7 6,2 11,7 16"/><polyline class="right" points="34 6,39 11,34 16"/><line x1="2" y1="11" x2="39" y2="11"/></svg></td><th class="no_label">min</th></tr> \
...
function getFieldValues(mySet) {
  
  var myFieldValuesArray = [];
  ...
  for (i in mySet.meta['fields']) {
    ...
    var myFieldObj = {};
    myFieldObj.id = 'cue'+i;
    myFieldObj.name = myField;
    myFieldObj.yes = myFieldMax;
    myFieldObj.no = myFieldMin;
    myFieldObj.split = myFieldMean;
    myFieldValuesArray.push(myFieldObj);
  }
  
  //console.log('HERE MEANS! myFieldValuesArray: '+JSON.stringify(myFieldValuesArray, null, "  "));

  return myFieldValuesArray;
}

function convertToBinary(mySet, mySplitValuesArray) {
    
  // pick a field
  for (i in mySplitValuesArray) {
    //var myField = mySplitValuesArray[i].name;
    ...
    mySet.data.forEach(function(myObj) {
      console.log('HERE CHECK! myFieldValues: '+JSON.stringify(myFieldValues, null, "  "));
      console.log('HERE CHECK! myObj: '+JSON.stringify(myObj, null, "  "));
  ...
  // add the split values to the object !!! FIX THIS!!!
  mySet.split_values = mySplitValuesArray;
  
  console.log('HERE BINARY! mySet: '+JSON.stringify(mySet, null, "  "));
